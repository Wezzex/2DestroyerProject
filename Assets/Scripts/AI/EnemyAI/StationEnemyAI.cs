...
public class StationEnemyAI : DefaultEnemyAI
{
    ...
    [SerializeField] protected Transform stationTransform;
    ...
    {
        if (Vector3.Distance(stationTransform.transform.position, aIDetector.Target.transform.position) < destroyerFieringRadius)
        {
            bInFireRange = true;
            return true;
        }
