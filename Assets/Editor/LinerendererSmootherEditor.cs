...
public class LinerendererSmootherEditor : Editor
{
   private LineRendererSmoother Smoother;
    ...
    private bool bExpandCurve = false;
    ...
    public override void OnInspectorGUI()
    {
        ...
        { 
            ...
            bool bLineRendererPathAndInitialStateAreSame = Smoother.Line.positionCount == Smoother.InitialState.Length;

            if (bLineRendererPathAndInitialStateAreSame)
            {
                Vector3[] positions = new Vector3[Smoother.Line.positionCount];
                Smoother.Line.GetPositions(positions);

                bLineRendererPathAndInitialStateAreSame = positions.SequenceEqual(Smoother.InitialState);
            }

            GUI.enabled = !bLineRendererPathAndInitialStateAreSame;
    ...
    private void SmoothPath()
    {
        ...
        SmoothingLength.floatValue = 0.0f;
        serializedObject.ApplyModifiedProperties();
    }
    ...
    private void DrawSegments() 
    {
        for (int i = 0; i < Curves.Length; i++)
        {
            ...
            for (int j = 0; j < segments.Length; j++)
            {
                ...
                Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), segments[j], Quaternion.identity, 0.25f, EventType.Repaint);
            }

            Handles.color = Color.white;
            Handles.Label(segments[segments.Length - 1], $"´C{i} S{segments.Length - 1}");
            Handles.DotHandleCap(EditorGUIUtility.GetControlID(FocusType.Passive), segments[segments.Length - 1], Quaternion.identity, 0.25f, EventType.Repaint);
